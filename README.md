...
///////////////////
## Задание №4 <br />
4 Реализовать физический уровень приемника SPI Peripheral с поддержкой операции записи.
...
Файл с исходным кодом - zad4.v <br />

Описание программы: <br />
Входные данные:<br />
wire - mosi, куда поступает командное слово по биту.<br />
CS - выбор слейва.<br />
Выходные данные:<br />
data_out[31:0] - полученные из моси данные.<br />
address_out[23:0] - полученные из моси адрес.<br />
wr_en_out - флаг выдачи даты и адресса. <br />
Регистры: <br />
bit_count [5:0] - счетчик битов для записи полного слова.<br />
command_wr_en - значение на разрешение записи для сравнения с полученной командой. <br />
mem - внутренний сдвиговый регистр для хранения mosi.

Алгоритм: <br />
На линию mosi с поднятием CS поступают изменяющиеся биты от 0 до 1 в течении 64 тактов. <br />
Данные из линии mosi записываются в mem[63:0] со сдвигом. <br />
При поднятом флаге CS происходит счет тактов в bit_count, если CS поднят 64 такта, данные записанные в mem парсятся на data_out, addres_out и сравниваются command_wr_en. <br />
В коде определенна команда на выдачу данных как command_wr_en, если пришедшая комадна совпадает с переменной, на выход slave поступает дата и адресс. <br />
При опускании CS bit_count обнуляется. <br />

![image](https://github.com/rufiano/verilog/assets/122745439/a536f8a8-063d-464d-bd87-7ee0f2ae0a4d)
